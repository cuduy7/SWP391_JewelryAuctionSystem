...
const TablePost: React.FC<TablePostProps> = ({ listItem }) => {
  ...
  const listAction = [
    {
      title: 'Xem bài viết',
      src: (postId: string) => router.push(`/product/detail-product/${postId}`),
    },
    {
      title: 'Xoá bài viết',
      src: (postId: string) => {
      ...
      <thead>
        <tr>
          {listTitleUserDetail.map((item, index) => (
            <th
              className="font-semibold py-3 border-b border-black border-opacity-10"
              key={index}
            >
              {item.title}
            </th>
          ...
          <tr key={index}>
            ...
            <td className="py-3 relative">
              ...
              {showToggleItemID === index && (
                <div
                  className="absolute right-[13rem] md:right-[17rem] lg:right-[17rem] sm:bottom-4 bottom-5 bg-gray-100 shadow-md rounded-lg w-auto translate-x-full translate-y-full transition p-2 z-[1001] text-left whitespace-nowrap"
                  ref={ref}
                >
                  <ul className="space-y-2 list-none">
                    {listAction.map((action, index) => (
                      <li
                        className="hover:bg-slate-200 hover:text-primary-blue-cus p-2 cursor-pointer"
                        key={index}
                      >
                        <button
                          type="button"
                          onClick={() => action.src(item.id ? item.id : '')}
                        >
                          {action.title}
                        </button>
                      </li>
...
const UserDetailManage = () => {
  ...
  const { data: listPostForUser, error } = useSWR<UserDetailManage>(
    user && user.id && id
      ? `/api/users/admin/${user.id}/user/${id}/detail`
      : null,
    fetcher,
  ...
  const pageCount = Math.ceil(
    listPostForUser && listPostForUser.data && listPostForUser.data.posts
      ? listPostForUser.data.posts.length / itemsPerPage
      : 0,
  );
  ...
  const visibleItems =
    listPostForUser &&
    listPostForUser.data &&
    listPostForUser.data.posts &&
    listPostForUser.data.posts.length > 0
      ? listPostForUser.data.posts.slice(startIndex, endIndex)
      : [];

  const checkAdminToUser =
    user &&
    user.role &&
    user.role.toLowerCase() === 'admin' &&
    listPostForUser &&
    listPostForUser.data.role.toString().toLowerCase() === 'user';
  const checkAdminToStaff =
    user &&
    user.role &&
    user.role.toLowerCase() === 'admin' &&
    listPostForUser &&
    listPostForUser.data.role.toString().toLowerCase() === 'staff';
  const checkStaffToUser =
    user &&
    user.role &&
  ...
  return (
    <div className="relative flex flex-col px-6 py-10 gap-5">
      ...
      >
        ...
        <h1 className="md:text-4xl text-3xl">Thông tin chi tiết người dùng</h1>
      </div>
      ...
      <div className="flex flex-col gap-5">
        ...
        <div className="relative flex gap-3">
          ...
          {(checkAdminToUser || checkStaffToUser || checkAdminToStaff) &&
            listPostForUser &&
            listPostForUser.data.isBanded === true && (
              <Button
                title="Mở khoá"
          ...
          {(checkAdminToUser || checkStaffToUser || checkAdminToStaff) &&
            listPostForUser &&
            listPostForUser.data.isBanded === false && (
              <Button
